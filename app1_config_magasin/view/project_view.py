from PyQt5.QtWidgets import (
    QWidget, QLabel, QPushButton, QLineEdit, QFileDialog, QVBoxLayout, QHBoxLayout, QSpinBox, QGridLayout, QMessageBox, QComboBox
)
...
class ProjectView(QWidget):
    ...
    def display_image_with_grid(self, path, nb_cols=20, nb_rows=15):
        ...
        width = scaled_pixmap.width(); height = scaled_pixmap.height()
        grid_width = width // nb_cols; grid_height = height // nb_rows
        painter = QPainter(scaled_pixmap)
        pen = QPen(Qt.red); pen.setWidth(1); painter.setPen(pen)
        font = QFont(); font.setPointSize(8); painter.setFont(font)
        for x in range(0, width, grid_width): painter.drawLine(x, 0, x, height)
        for y in range(0, height, grid_height): painter.drawLine(0, y, width, y)
        for row in range(nb_rows):
            for col in range(nb_cols):
                label = chr(65 + col % 26) + str(row + 1)
                x = col * grid_width + 4
